...
class RequestsClassFour():
    def __init__(self, _store_name, _site_url=None, _extra_vars=None, **kwargs):
        self.store_name = _store_name
        self.all_urls = {}
        self.site_url = _site_url
        self.branches_dropdown_id, self.default_pop_item = (_extra_vars.values() if _extra_vars else (None, None))
        self.branches = {}
    ...
    def set_branch_single(self, branch_name, catID="PriceFull", sort="Time", sortdir="ASC", request_method="GET"):
        ...
        candidates = []
        try:
            numeric_id = int(storeId)
        except Exception:
            return False

        for width in [4, 3, 2, 1]:
            if width == 1:
                storeId_str = str(numeric_id)
            else:
                storeId_str = f"{numeric_id:0{width}d}"

            branch_token = f"-{storeId_str}-"
            matches = [fn for fn in files if branch_token in str(fn) and str(fn).startswith(str(catID))]
        ...
        # Extract date from filename (second to last dash-separated part)
        parts = chosen_filename.split("-")
        if len(parts) == 3:
            date_folder = parts[-1].split(".")[0][:8]
        elif len(parts) == 5:
            date_folder = parts[-2]
        else:
            return False
